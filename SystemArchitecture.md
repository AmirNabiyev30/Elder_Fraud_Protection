...
```mermaid
flowchart LR
    WC[Web Client: Chrome Extension] -->|HTTPS / JSON| WS[Web Server (planned: Node/Express or Nginx)]
    WS -->|HTTP / JSON| AS[App Server: Flask API]
    AS -->|PyMongo BSON/JSON| DB[(MongoDB Atlas)]
```
